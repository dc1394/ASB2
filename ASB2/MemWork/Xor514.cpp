...
namespace memwork {
	namespace myrandom {
		...
		{
			enum { a = 101, b = 99, c = 8 };
			const __m128i x = p.m[1];
			const __m128i w = p.m[4];
			__m128i s = _mm_slli_si128(_mm_slli_epi64(x, a - 64), 8);
			__m128i t = _mm_xor_si128(x, s);
			p.m[1] = p.m[2];
			p.m[2] = p.m[3];
			p.m[3] = w; 
		...
		__m128i Xor514::rand128()
		{
			...
			p.u[0] += 4;
			return _mm_load_si128(reinterpret_cast<const __m128i * const>(p.u + Xor514::XOR514HEAD));
		}
	}
}
